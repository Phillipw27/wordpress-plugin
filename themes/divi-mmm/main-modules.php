...
class ET_Builder_Module_Image extends ET_Builder_Module {
	function init() {
		...
		$this->fields_defaults = array(
			...
			'always_center_ƒon_mobile' => array( 'on' ),
...
class ET_Builder_Module_BreadCrumbs extends ET_Builder_Module{
	function init(){
		$this->name = esc_html__( 'BreadCrumbs', 'et_builder' );
		$this->slug = 'breadcrumb';
		$this->fullwidth = true;
		$this->whitelisted_fields = array();
		$this->custom_css_options = array();
	}
	...
	function shortcode_callback($atts, $content = null, $function_name){
		$module_class = ET_Builder_Element::add_module_order_class($module_class, $function_name);
		$output = '<div class="'.$module_class.'"><h1>BreadCrumbs</h1></div>';

return $output; 
	}
		}
		new ET_Builder_Module_BreadCrumbs;
		$module_class = ET_Builder_Element::add_module_order_class( $module_class, $function_name );
								...
								if ( 'on' === $args['show_content'] ) {
									...
									if ( has_excerpt() ) {
										...
										if ( '' !== $post_content ) {
											...
											echo wpautop( et_delete_post_first_video( truncate_post( 150, false, '', true ) ) );
