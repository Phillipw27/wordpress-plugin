...
class BWGModelComments_bwg {
  ...
  public function get_rows_data() {
    ...
    // $where .= ' AND t2.gallery_id=' . (isset($_POST['search_select_gal_value']) ? (int) $_POST['search_select_gal_value'] : 0);
    ...
    $order_by = ' ORDER BY ' . ((isset($_POST['order_by']) && esc_html($_POST['order_by']) != '') ? esc_html($_POST['order_by']) : 't1.name') . ' ' . $asc_or_desc;
    ...
    $rows = $wpdb->get_results("SELECT t1.*, t2.thumb_url, t2.alt FROM " . $wpdb->prefix . "bwg_image_comment as t1 INNER JOIN " . $wpdb->prefix . "bwg_image as t2 on t1.image_id=t2.id " . $where . $order_by . " LIMIT " . $limit . ",".$this->per_page);
    return $rows;
  }
