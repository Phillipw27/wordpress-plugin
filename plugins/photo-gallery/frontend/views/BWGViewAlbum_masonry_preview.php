  ...
  public function display($params, $from_shortcode = 0, $bwg = 0) {
                ...
                foreach ($album_galleries_row as $album_galallery_row) {
                  ...
                  if ($type != 'gallery') {                    
										...
										<span class="bwg_masonry_thumb_spun_<?php echo $bwg; ?>">
											...
											<a style="font-size: 0;" <?php echo ($from !== "widget" ? "href='" . str_replace("/thumb/", "/", $preview_url) . "' onclick=\"spider_frontend_ajax('gal_front_form_" . $bwg . "', '" . $bwg . "', 'bwg_album_masonry_" . $bwg . "', '" . $album_galallery_row->alb_gal_id . "', '" . $album_gallery_id . "', '" . $def_type . "', '', '" . $title . "', 'default'); return false;\"" : "href='" . $permalink . "'") ?>>
												
												<input id='<?php echo $album_galallery_row->alb_gal_id; ?>' type='hidden' value="<?php echo $title; ?>" />
												<img class="bwg_masonry_thumb_<?php echo $bwg; ?>" src="<?php echo $preview_url; ?>" alt="<?php echo $title; ?>" alt="<?php echo $title; ?>" style="width: <?php echo $params['masonry_album_thumb_width'];?>px; max-height: none !important; max-width: none !important;" />
									...
									?>
										<span class="bwg_masonry_thumb_spun_<?php echo $bwg; ?>">
											<a style="font-size: 0;" <?php echo ($params['thumb_click_action'] == 'open_lightbox' ? ('href="' . ($is_video ? $image_row->thumb_url : site_url() . '/' . $WD_BWG_UPLOAD_DIR . $image_row->image_url) . '" onclick="spider_createpopup(\'' . addslashes(add_query_arg($params_array, admin_url('admin-ajax.php'))) . '\', ' . $bwg . ', ' . $params['popup_width'] . ', ' . $params['popup_height'] . ', 1, \'testpopup\', 5); return false;"') : ('href="' . $image_row->redirect_url . '" target="' .  ($params['thumb_link_target'] ? '_blank' : '')  . '"')) ?>>
												<img class="bwg_masonry_thumb_<?php echo $bwg; ?>" id="<?php echo $image_row->id; ?>" src="<?php echo ($is_video ? "" : site_url() . '/' . $WD_BWG_UPLOAD_DIR) . $image_row->thumb_url; ?>" alt="<?php echo $image_row->alt; ?>" style="width: <?php echo $params['masonry_album_image_thumb_width'];?>px; max-height: none !important;  max-width: none !important;" />
												<?php if ($options_row->show_masonry_thumb_description) : ?>
													<div class="bwg_masonry_thumb_description_<?php echo $bwg; ?>">
    ...
    <script>
      ...
      if (bwg_hash && bwg_hash.indexOf("bwg") != "-1") {
        ...
        spider_createpopup('<?php echo addslashes(add_query_arg($params_array_hash, admin_url('admin-ajax.php'))); ?>&gallery_id=' + bwg_hash_array[0] + '&image_id=' + bwg_hash_array[1], '<?php echo $bwg; ?>', '<?php echo $params['popup_width']; ?>', '<?php echo $params['popup_height']; ?>', 1, 'testpopup', 5);
      }
      <?php
      }
      ?>
