    ...
    <div class="bwg_image_wrap">
      ...
      <div class="bwg_btn_container">
        <div class="bwg_ctrl_btn_container">
          ...
          <?php } 
          $share_url = add_query_arg(array('action' => 'Share', 'curr_url' => $current_url, 'image_id' => $current_image_id), admin_url('admin-ajax.php')) . '#bwg' . $gallery_id . '/' . $current_image_id;
          $share_image_url = urlencode(($current_filetype == "YOUTUBE" || $current_filetype == "VIMEO") ? $current_thumb_url : site_url() . '/' . $WD_BWG_UPLOAD_DIR . $current_image_url);
          ...
          if ($enable_image_twitter) {
            ?>
            <a id="bwg_twitter_a" href="https://twitter.com/share?url=<?php echo urlencode($share_url); ?>" target="_blank" title="<?php echo __('Share on Twitter', 'bwg'); ?>">
          ...
          if ($enable_image_pinterest) {
            ?>
            <a id="bwg_pinterest_a" href="http://pinterest.com/pin/create/button/?s=100&url=<?php echo urlencode($share_url); ?>&media=<?php echo $share_image_url; ?>&description=<?php echo $current_image_description; ?>" target="_blank" title="<?php echo __('Share on Pinterest', 'bwg'); ?>">
                ...
                <form id="bwg_rate_form" method="post" action="<?php echo $popup_url; ?>">
                  ...
                  <script>
                    function bwg_rating(current_rate, rate_count, avg_rating, cur_key) {
                      ...
                      if (typeof jQuery().raty !== 'undefined' && jQuery.isFunction(jQuery().raty)) {
          ...
          <div id="bwg_comments">
            ...
            <form id="bwg_comment_form" method="post" action="<?php echo $popup_url; ?>">
              ...
              <p><input onclick="if (spider_check_required('bwg_name', '<?php echo __('Name', 'bwg'); ?>') <?php if ($option_row->popup_enable_email) { ?> || spider_check_required('bwg_email', '<?php echo __('Email', 'bwg'); ?>') || spider_check_email('bwg_email') <?php } ?> || spider_check_required('bwg_comment', '<?php echo __('Comment', 'bwg'); ?>')) { return false;}
                                 ...
                                 return false;"
                        ontouchend="if (spider_check_required('bwg_name', '<?php echo __('Name', 'bwg'); ?>') <?php if ($option_row->popup_enable_email) { ?> || spider_check_required('bwg_email', '<?php echo __('Email', 'bwg'); ?>') || spider_check_email('bwg_email') <?php } ?> || spider_check_required('bwg_comment', '<?php echo __('Comment', 'bwg'); ?>')) {return false;}
                                 var cur_image_key = parseInt(jQuery('#bwg_current_image_key').val());
                                 ++data[cur_image_key]['comment_count'];
                                 spider_set_input_value('ajax_task', 'save');
                                 spider_set_input_value('image_id', jQuery('#bwg_popup_image').attr('image_id'));
      ...
      function bwg_change_image(current_key, key, data, from_effect) {
        ...
        if (typeof data[key] != 'undefined' && typeof data[current_key] != 'undefined') {
          ...
          function bwg_afterload() {
            ...
            var bwg_share_url = encodeURIComponent("<?php echo add_query_arg(array('action' => 'Share', 'curr_url' => $current_url, 'image_id' => ''), admin_url('admin-ajax.php')); ?>") + "=" + data[key]['id'] + encodeURIComponent('#bwg<?php echo $gallery_id; ?>/') + data[key]['id'];
            if (data[key]['filetype'] == "YOUTUBE" || data[key]['filetype'] == "VIMEO") {
              var bwg_share_image_url = encodeURIComponent(data[key]['thumb_url']);
            }
            else {
              var bwg_share_image_url = "<?php echo urlencode(site_url() . '/' . $WD_BWG_UPLOAD_DIR); ?>" + data[key]['image_url'];
            }
